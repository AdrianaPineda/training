...
function findNextBiggestString(baseCharCode, string) {
    const { char: nextHighestChar, index: nextHighestCharIndex } = getNextHighestCharIndex(baseCharCode, string);
    if (nextHighestCharIndex >= 0) {
        const remainingString = removeChar(string, nextHighestCharIndex);
        const orderedString = orderAlphabetical(remainingString);
        return nextHighestChar + orderedString;
    }
